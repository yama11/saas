        ...
        <el-row>
          ...
          >
            ...
            <el-time-picker
              ...
              :default-value="[new Date( initDate + ' 00:00:00'),new Date( initDate + ' 23:59:00')]"
              arrow-control
              clearable
              is-range
              format="HH:mm"
 ...
 */
  ...
  methods: {
    addInput(countIndex) {
      const index_time = this.weekList.findIndex(item => countIndex === item.day);
    ...
    changeCycle(listData, index, count) {
      ...
      const data_val = this.weekList.find(item => item.day === currDay);
      const data_index = this.weekList[index].timeVal.findIndex(item => (item.start_end === null));
    ...
    getWeekList(cycleArr) {
      ...
      cycleArr.forEach((itemCycle) => {
        ...
        const index_time = weeekList.findIndex(itemWeek => itemCycle.day === itemWeek.day);
    ...
    getCyclesArr(cycleArr, item, oneTime) {
      if (oneTime.start_end instanceof Array) {
        cycleArr.push({
          day: item.day,
